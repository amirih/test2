		...
		return new GeomPortrayal() {
			public void draw(Object object, Graphics2D graphics, DrawInfo2D info) {
				...
				*/
				...
				if(person.getExposedDays()>0){paint=ColorUtils.getAgentHungerColorMap().get(11);}
				else if(person.getSickDays()>0){paint=ColorUtils.getAgentHungerColorMap().get(0);}
				else if(person.getResistDays()>0){paint=ColorUtils.getAgentHungerColorMap().get(10);}
				else{paint=ColorUtils.getAgentHungerColorMap().get(8);}
				super.draw(object, graphics, info);
			}
		};
	}
